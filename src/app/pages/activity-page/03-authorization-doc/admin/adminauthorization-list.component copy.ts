...
import { OtoApprovalModalComponent } from '../authorization-list/modal/approval/otoapproval-modal.component';
import { OtoHistoryModalComponent } from '../authorization-list/modal/history/otohistory-modal.component';

@Component({
    selector: 'app-authorization-list',
    templateUrl: './adminauthorization-list.component.html',
    styleUrl: './adminauthorization-list.component.scss',
    animations: [
        trigger('fadeIn', [
            transition(':enter', [
                style({ opacity: 0 }),
...
export class AdminAuthorizationListComponent implements OnInit, OnDestroy {
    pageSize = 10;
    page = 1;
    searchTerm = '';
    sortColumn = 'id';
    ...
    ngOnInit(): void {
        ...
        if (roleName === 'Admin' || roleName === 'Super Admin') {
            this.pageTitle = `${roleName} AuthDoc Activity`;
            this.cardTitle = `${roleName} Auth Doc List`;
    ...
    loadedProposedChanges(): void {
        ...
        this.service.get(url).pipe(
            ...
            error: (error) => this.handleError(error, 'Error fetching authdoc data')
        });
    }
