...
@Component({
  ...
  templateUrl: './list-document-number.component.html',
  styleUrl: './list-document-number.component.scss'
})
export class ListDocumentNumberComponent implements OnInit {
    // Properti
    pageSize = 10;
    page = 1; 
    searchTerm = '';
    ...
    ngOnInit(): void {
        ...
        this.loadedDocumentNumbers(); // Memuat data awal
    }
    ...
    loadedDocumentNumbers() {
        this.loading = true;
        const url = `/documentnumbers?page=${this.page}&limit=${this.pageSize}&search=${this.searchTerm}&sort=${this.sortColumn}&direction=${this.sortDirection}`; // Sertakan sort dan direction
    ...
    //  Setelah mengubah sortColumn dan sortDirection, panggil loadedDocumentNumbers()
    this.loadedDocumentNumbers();
    }



    public formData = this.fb.group({
        document_type: ['', [Validators.required, Validators.minLength(3), Validators.maxLength(255)]],
        document_code: ['', [Validators.required, Validators.minLength(1), Validators.maxLength(255)]],
...
onSubmit() {
    ...
    if (this.statusForm === 'add') {
        ...
        this.service.post('/documentnumbers', payload).subscribe({
            next: () => {
        ...
        this.service.put(`/documentnumbers/${this.idEdit}`, payload).subscribe({
            next: () => {
    ...
    onDelete(id: number) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
              this.loading = true;
                this.service.delete(`/documentnumbers/${id}`).subscribe({
                    next: () => {
                        Swal.fire(
                            'Deleted!',
