...
@Component({
    selector: 'app-list-document-number',
    templateUrl: './list-document-number.component.html',
    styleUrls: ['./list-document-number.component.scss'],
    animations: [
        trigger('fadeIn', [
            transition(':enter', [
                style({ opacity: 0 }),
...
export class ListDocumentNumberComponent implements OnInit, OnDestroy {
    ...
    onNavigate(): void {
        this.router.navigate(['activity-page/form-number']);
    }
    ...
    redirectToFormProposedChanges(id: number): void {
        this.loading = true;
        // Simpan ID ke sessionStorage jika diperlukan
        sessionStorage.setItem('formId', id.toString());
      
        // Navigasi ke proposedchanges-form tanpa id
    ...
    onDelete(id: number): void {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                this.loading = true;
                this.service.delete(`/documentnumbers/${id}`).pipe(
                    finalize(() => this.loading = false)
                ).subscribe({
                    next: () => {
