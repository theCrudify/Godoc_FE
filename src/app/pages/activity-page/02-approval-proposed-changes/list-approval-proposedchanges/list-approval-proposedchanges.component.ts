...
@Component({
    selector: 'app-list-approval-proposedchanges',
    templateUrl: './list-approval-proposedchanges.component.html',
    styleUrl: './list-approval-proposedchanges.component.scss',
    animations: [
        trigger('fadeIn', [
            transition(':enter', [
                style({ opacity: 0 }),
...
export class ListApprovalProposedchangesComponent implements OnInit, OnDestroy {
    ...
    initBreadcrumbs(): void {
        this.breadCrumbItems = [
            { label: 'Activity' },
            { label: 'Approval Proposed Changes', active: true }
        ];
    }

    loadedProposedChanges(): void {
        this.loading = true;
        const url = `/ongoing?approval_auth_id=${this.GodocUser.auth_id}&page=${this.page}&limit=${this.pageSize}&search=${this.searchTerm}&sort=${this.sortColumn}&direction=${this.sortDirection}`;
    ...
    onView(id: number): void {
        ...
        // Contoh implementasi
        this.router.navigate(['activity-page/approval-detail/', id]);
    }
