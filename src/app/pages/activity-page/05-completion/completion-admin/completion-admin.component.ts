...
@Component({
    selector: 'app-completion-admin',
    templateUrl: './completion-admin.component.html',
    styleUrl: './completion-admin.component.scss',
...
export class completionAdminComponent implements OnInit, OnDestroy {
    pageSize = 10;
    page = 1;
    searchTerm = '';
    sortColumn = 'id';
    ...
    ngOnInit(): void {
        ...
        const roleName = this.GodocUser?.role?.role_name?.toLowerCase();
    ...
    loadedProposedChanges(): void {
        ...
        if (roleName === 'super admin') {
            url = `/completion?page=${this.page}&limit=${this.pageSize}&search=${this.searchTerm}&sort=${this.sortColumn}&direction=${this.sortDirection}`;
        } else if (roleName === 'admin') {
            url = `/completion?department_id=${this.GodocUser.section.department_id}&page=${this.page}&limit=${this.pageSize}&search=${this.searchTerm}&sort=${this.sortColumn}&direction=${this.sortDirection}`;
        } else {
            this.router.navigate(['/activity-page/notfound']);
            return;
        }
...
// In completion-list.component.ts 

// MODIFY: processDataForDisplay method to update how width is calculated
    ...
    for (const item of data) {
        ...
        try {
            ...
            this.progressData.set(item.id, {
                ...
                handoverProgress,
                totalProgress,
                // CHANGED: Each segment now gets 25% width - all segments are always shown
                mainWidth: '25%',
                supportWidth: '25%',
                authWidth: '25%',
                handoverWidth: '25%',
                // Store the actual progress percentages for fill calculations
                mainFillPercentage: (mainProgress / 100) * 100,
                supportFillPercentage: (supportProgress / 100) * 100,
                authFillPercentage: (authProgress / 100) * 100,
                handoverFillPercentage: (handoverProgress / 100) * 100,
...
// MODIFY: the empty progress data method to include the new properties
getEmptyProgressData(): any {
    return {
        ...
        handoverProgress: 0,
        totalProgress: 0,
        mainWidth: '25%',   // Always 25%
        supportWidth: '25%', // Always 25%
        authWidth: '25%',    // Always 25%
        handoverWidth: '25%', // Always 25%
        mainFillPercentage: 0,
        supportFillPercentage: 0,
        authFillPercentage: 0,
        handoverFillPercentage: 0,
