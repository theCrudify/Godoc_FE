...
@Component({
  ...
  selector: 'app-completionapproval-modal',
  templateUrl: './completionapproval-modal.component.html',
  styleUrls: ['./completionapproval-modal.component.scss']
})
export class ApprovalCompletionComponent implements OnInit {
  ...
  loadApprovalData(): void {
    ...
    this.service.get(`/approval-byID-handover/${this.id}`).pipe(
  ...
  getStars(rating: number): number[] {
    const maxStars = 5;
    return Array(Math.min(rating, maxStars)).fill(0);
  }
