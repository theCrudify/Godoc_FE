...
@Component({
    selector: 'app-handover-list-approver',
    templateUrl: './handover-list-approver.component.html',
    styleUrl: './handover-list-approver.component.scss',
    animations: [
        trigger('fadeIn', [
            transition(':enter', [
                style({ opacity: 0 }),
...
export class handoverListApproverComponent implements OnInit, OnDestroy {
    pageSize = 10;
    page = 1;
    searchTerm = '';
    sortColumn = 'id';
    ...
    loadedProposedChanges(): void {
        this.loading = true;
        const url = `/ongoinghandover?approval_auth_id=${this.GodocUser.auth_id}&page=${this.page}&limit=${this.pageSize}&search=${this.searchTerm}&sort=${this.sortColumn}&direction=${this.sortDirection}`;
    ...
    onView(id: number): void {
        ...
        console.log(`View item with ID: ${id}`);
        // Contoh implementasi
        this.router.navigate(['activity-page/handover-approver/', id]);
    }
