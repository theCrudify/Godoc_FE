...
export class MasterAreaComponent implements OnInit {
    ...
    loadedAreas() {
        this.loading = true;
        const url = `/areas?page=${this.page}&limit=${this.pageSize}&search=${this.searchTerm}&sort=${this.sortColumn}&direction=${this.sortDirection}`; //Sertakan sort dan direction
    ...
    public formData = this.fb.group({
        area: ['', [Validators.required, Validators.minLength(3), Validators.maxLength(255)]],
        code_area: ['', [Validators.required, Validators.pattern('^[A-Z0-9_-]+$')]],
        id_line: [null],
        status: [true]
    });
